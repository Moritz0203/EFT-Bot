				...
				if (!ReturnDataCase.empty()) {
					...
					for (int i3 = 0; i3 < ReturnDataCase_Clean.size(); i3++) {
						...
						if (checkSecondLastChar(tagCase)) {
							...
							{
							case 0:
								pointCasetempStashTemp.emplace_back(ReturnDataCase_Clean[i3], NameOfItemCases[i], tagCase, templ.rows, templ.cols, i1, 0x01, freeSlots_empty, prefix);
							default:
								pointCasetempStashTemp.emplace_back(ReturnDataCase_Clean[i3], NameOfItemCases[i], tagCase, templ.rows, templ.cols, i1, identyfierAsHEX_ST, freeSlots_empty, prefix);
							}
